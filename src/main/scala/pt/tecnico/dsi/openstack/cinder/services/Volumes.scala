 ...
 *
    ...
    */
  def listSummary(query: Query = Query.empty): Stream[F, VolumeSummary] = super.list[VolumeSummary](pluralName, uri, query)

  override def list(query: Query, extraHeaders: Header*): Stream[F, Volume] =
    super.list[Volume](pluralName, uri / "detail", query, extraHeaders:_*)
